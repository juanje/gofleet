...
public class MemoryTileCache implements TileCache {
	...
	protected static int globalCacheSize = 200;
	...
	public MemoryTileCache() {
		...
		globalCacheSize = LogicConstants.getInt("MAXGLOBALTILESCACHED", 500);
		globalSize++;
	}

	public static void resetGlobalSize() {
		globalSize = 0;
	}
	...
	public void addTile(Tile tile) {
		...
		if (hashtable.size() > cacheSize
				|| globalSize() * hashtable.size() > globalCacheSize)
			removeOldEntries();
	}
	...
	protected void removeOldEntries() {
		synchronized (lruTiles) {
			try {
				while (lruTiles.getElementCount() > 3 * cacheSize / 4
						&& lruTiles.getElementCount() > 10) {

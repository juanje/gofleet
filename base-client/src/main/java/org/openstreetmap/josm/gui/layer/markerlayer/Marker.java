...
public class Marker implements ActionListener {
	public static final int LABEL_ZOOM_THRESHOLD = LogicConstants.getInt("LABEL_ZOOM_THRESHOLD");

    public EastNorth eastNorth;
    public Color color = Color.BLACK, bgcolor = Color.WHITE; // Color for text and text bg
    ...
    static {
        Marker.markerProducers.add(new MarkerProducers() {
            public Marker createMarker(WayPoint wpt, File relativePath, MarkerLayer parentLayer, double time, double offset) {
                ...
                if (uri == null) {
                	Marker m = new Marker(wpt.latlon, name_desc, wpt.getString("symbol"), parentLayer, time, offset);
                	if (wpt.attr.containsKey("color"))
                		m.color = Color.decode(wpt.getString("color"));
                	if (wpt.attr.containsKey("bgcolor"))
                		m.bgcolor = Color.decode(wpt.getString("bgcolor"));
                    return m;
    ...
    public Marker(LatLon ll, String text, String iconName, MarkerLayer parentLayer, double time, double offset) {
        ...
        this.offset = offset;
        this.time = time;
        Icon symbol = LogicConstants.getIcon(iconName);
        this.symbol = symbol;
    ...
    public boolean containsPoint(Point p) {
		boolean hit = screen != null
				&& this.symbol != null
				&& (screen.distance(p) < Math.max(
						this.symbol.getIconHeight() / 2, this.symbol
								.getIconWidth() / 2));
		return hit;
    }
...
//    	System.out.println(this.toString() + " hit!");
    }
    ...
    public void paint(Graphics g, MapView mv, boolean mousePressed, String show) {
        screen = mv.getPoint(eastNorth);
        g.setColor(color);
        int ih, iw;
        if (symbol != null && show.equalsIgnoreCase("show")) {
        	ih = symbol.getIconHeight();
        	iw = symbol.getIconWidth();
            symbol.paintIcon(mv, g, screen.x-iw/2, screen.y-ih/2);
        } else {
        ...
        if (mv.zoom() >= LABEL_ZOOM_THRESHOLD &&
        		(text != null) && (show.equalsIgnoreCase("show"))){

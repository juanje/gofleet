...
public class OsmDataLayer extends Layer {
	...
	public void mergeFrom(final Layer from) {
		...
		if (data.version == null) {
			...
			if ("0.5".equals(data.version)
					^ "0.5".equals(((OsmDataLayer) from).data.version)) {
				System.err
						.println("Warning: mixing 0.6 and 0.5 data results in version 0.5");
				data.version = "0.5";
			}
		}
		fireDataChange();
	...
	public Object getInfoComponent() {
		...
		for (int i = 0; i < counter.normal.length; ++i) {
			String s = counter.normal[i]
					+ " "
					+ trn(counter.names[i], counter.names[i] + "s",
							counter.normal[i]);
			if (counter.deleted[i] > 0)
				s += tr(" ({0} deleted.)", counter.deleted[i]);
			p.add(new JLabel(s, ImageProvider.get("data", counter.names[i]),
					JLabel.HORIZONTAL), GBC.eop().insets(15, 0, 0, 0));
		}
		p.add(new JLabel(tr("API version: {0}",
				(data.version != null) ? data.version : tr("unset"))));
	...
	public class ConvertToGpxLayerAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			Main.main.addLayer(new GpxLayer(toGpxData(), tr(
					"Converted from: {0}", name)));

...
public class LatLon extends Coordinate {
	...
	/**
	 * Minimum difference in location to not be represented as the same
	 * position. The API returns 7 decimals.
	 */
	 ...
	 *         values, only differing by no more than 1 /
	 *         {@link #MAX_SERVER_PRECISION MAX_SERVER_PRECISION}.
	 */
	...
	/**
	 * Computes the distance between this lat/lon and another point on the
	 * earth. Uses Haversine formular.
	 * 
	 * @param other
	 *            the other point.
	...
	public double greatCircleDistance(LatLon other) {
		...
		double d = 2
				* R
				* asin(sqrt(sinHalfLat * sinHalfLat
						+ cos(toRadians(this.lat()))
						* cos(toRadians(other.lat())) * sinHalfLon * sinHalfLon));
	...
	/**
	 * Returns the heading, in radians, that you have to use to get from this
	 * lat/lon to another.
	 * 
	 * @param other
	...
	public LatLon getCenter(LatLon ll2) {
		return new LatLon((this.lat() + ll2.lat()) / 2.0, (this.lon() + ll2
				.lon()) / 2.0);
	}
	...
	public LatLon getRoundedToOsmPrecision() {
		return new LatLon(Math.round(lat() / MAX_SERVER_PRECISION)
				* MAX_SERVER_PRECISION, Math
				.round(lon() / MAX_SERVER_PRECISION)
				* MAX_SERVER_PRECISION);
	}
